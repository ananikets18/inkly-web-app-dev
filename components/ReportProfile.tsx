      ...
      <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
        ...
        >
          ...
          <p className="text-sm text-center text-zinc-600 dark:text-zinc-400 mb-6">
            Help us keep Inkly safe. Why are you reporting {profileName}?
          </p>
          ...
          <RadioGroup value={selectedReason} onValueChange={setSelectedReason} className="space-y-3">
            {reportReasons.map((reason) => (
              <div key={reason} className="flex items-center space-x-3">
                <RadioGroupItem value={reason} id={reason} className="border-zinc-300 dark:border-zinc-600" />
                <Label htmlFor={reason} className="text-sm text-zinc-700 dark:text-zinc-300">
                  {reason}
                </Label>
              </div>
            ))}
          ...
          {isOther && (
            <div className="mt-4">
              <Label htmlFor="customReason" className="block mb-2 text-sm text-zinc-700 dark:text-zinc-300">
          ...
          <Button
            ...
            disabled={!selectedReason || (isOther && (!customText.trim() || isOverLimit)) || isSubmitting}
